...
public class SwtJfxTimeGraphViewerStatesTest extends SwtJfxTimeGraphViewerTestBase {
    ...
    public void testStatesResolution() {
         ...
         */
        double viewWidth = getViewer().getTimeGraphScrollPane().getViewportBounds().getWidth();
        ...
        getViewer().prepareWaitForRepaint();
        getViewer().paintCurrentLocation();
        while (!getViewer().waitForRepaint()) {
            updateUI();
        }

        Collection<StateRectangle> renderedStates = getViewer().getRenderedStateRectangles();
