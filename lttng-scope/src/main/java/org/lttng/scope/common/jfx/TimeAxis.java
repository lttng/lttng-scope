...
/**
 * Rewrite of NumberAxis but with changes to {@link #calculateTickValues}
 * to handle time correctly.
 *
 * (why is NumberAxis final, would be much simpler to just extend it... the methods are protected ffs!)
 */
...
//        }
    ...
    protected List<Number> calculateTickValues(double length, Object range) {
        ...
        /* Aim to have at least 5 ticks in the range, but as few as possible above that. */
        // TODO
    ...
    private void computeMajorTicks(List<Number> majorTicks, double lowerBound, double upperBound, double tickUnit) {
        double firstTick = lowerBound - (lowerBound % tickUnit) + tickUnit;
        for (double tick = firstTick; tick <= upperBound; tick += tickUnit) {
            majorTicks.add(tick);
        }
    }
    ...
    protected List<Number> calculateMinorTickMarks() {
        ...
        final List<Number> majorTicks = new ArrayList<>();
        computeMajorTicks(majorTicks, lowerBound, upperBound, tickUnit);
        if (majorTicks.isEmpty()) {
            return Collections.emptyList();
        }

        /*
         * Simply add minor ticks in between the major ones.
         *
         * We will also add ticks before the first and after the last major ticks,
         * so that the view may start/end with minor ticks (ticks outside the visible
         * time range won't be rendered).
         */

        /* Add minor ticks before the first major one. */
        {
            /* We're going "backwards" here, then re-sorting after. */
            double firstMajorTick = majorTicks.get(0).doubleValue();
            double limit = firstMajorTick - tickUnit;
            List<Double> firstTicks = new ArrayList<>();

            double minorTick = firstMajorTick - minorUnit;
            while (minorTick > limit) {
                firstTicks.add(minorTick);
                minorTick -= minorUnit;
            }
            firstTicks.sort(Comparator.naturalOrder());
            minorTickMarks.addAll(firstTicks);
        }

        /* Add the minor ticks that are between two majors. */
        for (int i = 0; i < majorTicks.size() - 1; i++) {
            final double beginMajorTick = majorTicks.get(i).doubleValue();
            final double endMajorTick = majorTicks.get(i + 1).doubleValue();

            double minorTick = beginMajorTick + minorUnit;
            while (minorTick < endMajorTick) {
                minorTickMarks.add(minorTick);
                minorTick += minorUnit;
            }
        }

        /* Add the minor ticks after the last major one. */
        {
            final double lastMajorTick = majorTicks.get(majorTicks.size() - 1).doubleValue();
            final double limit = lastMajorTick + tickUnit;

            double minorTick = lastMajorTick + minorUnit;
            while (minorTick < limit) {
                minorTickMarks.add(minorTick);
                minorTick += minorUnit;
            }
