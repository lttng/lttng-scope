...
public class EventSeriesMenuButton extends MenuButton {
    ...
    public EventSeriesMenuButton(TimeGraphWidget widget) {
        ...
        addNewSeriesMenuItem.setOnAction(e -> {
            ...
            Optional<PredicateDrawnEventProvider> results = dialog.showAndWait();
            TimeGraphDrawnEventProvider provider = results.orElse(null);
            if (provider != null) {
        ...
        PROVIDER_MANAGER.getRegisteredProviders().addListener((SetChangeListener<TimeGraphDrawnEventProvider>) change -> {
    ...
    private void addProviderToMenu(TimeGraphDrawnEventProvider provider) {
        CheckMenuItem item = new EventProviderMenuItem(provider);
        int index = getItems().size() - 3;
        getItems().add(index, item);
    }

    private void removeProviderFromMenu(TimeGraphDrawnEventProvider provider) {
     ...
     * {@link TimeGraphDrawnEventProvider}.
     */
    private static class EventProviderMenuItem extends CheckMenuItem {

        private final TimeGraphDrawnEventProvider fProvider;

        public EventProviderMenuItem(TimeGraphDrawnEventProvider provider) {
            fProvider = provider;

            TimeGraphDrawnEventSeries series = provider.getDrawnEventSeries();
        ...
        public TimeGraphDrawnEventProvider getProvider() {
            return fProvider;
        }
    }
